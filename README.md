...
This GIT repo contains a Python script that can be used to forecast COVID-19 death and infections using case data from John Hopkins. The stats.py script will pull the current data (updated daily at 9a.m.), transform and aggregate the data using Pandas and generate a forecast using Facebook's Prophet library. Output is four forecast graphs in a PNG image file format. 
