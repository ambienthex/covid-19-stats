...
This GIT repo contains a Python(3) script (stats.py) that can be used to forecast U.S. COVID-19 deaths and infections using case data from John Hopkins. The stats.py script will pull the current John Hopkins COVID-19 case data (updated daily at 9a.m.), transform and aggregate the data using Pandas and generate a forecast using Facebook's Prophet library. Output is four forecast graphs in a PNG image file format. 
